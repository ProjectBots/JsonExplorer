...
import include.NEF;
import program.JsonExplorer.Search.SearchType;
...
public class JsonExplorer {
	...
	public static class Search {
		...
		public static enum SearchType {
			normal, advanced, regex;
		}

		JsonObject ret = new JsonObject();

		public JsonObject search(JsonElement je, String path) {
			if (je.isJsonObject()) {
				JsonObject jo = je.getAsJsonObject();
				for (String key : jo.keySet()) {
					test(jo, jo.get(key), key, path);
				}
			} else {
				JsonArray ja = je.getAsJsonArray();
				for (int i = 0; i < ja.size(); i++) {
					test(ja, ja.get(i), "" + i, path);
				}
			}
			return ret;
		}

		private JsonObject test(JsonElement parent, JsonElement e, String key, String path) {
			if (nested && (e.isJsonArray() || e.isJsonObject())) {
				search(e, path + "\\" + key);
			}

			// check for keys (or key value pairs if enabled)
			ifc: if (keys) {
				if ((!primitives && e.isJsonPrimitive()) || (!arrays && e.isJsonArray())
						|| (!objects && e.isJsonObject()) || (!nulls && e.isJsonNull()))
					break ifc;

				switch (type) {
				case advanced:
					if (!advMatch(term, key) || (matchVal != null && !advMatch(matchVal, e.toString())))
						break ifc;
					break;
				case normal:
					if (!key.toLowerCase().contains(term.toLowerCase())
							|| (matchVal != null && !e.toString().toLowerCase().contains(matchVal.toLowerCase())))
						break ifc;
					break;
				case regex:
					if (!Pattern.matches(term, key) || (matchVal != null && !Pattern.matches(matchVal, e.toString())))
						break ifc;
					break;
				}
			...
			// check for values
			// checked for key - value pairs (if enabled) before
			ifc: if (values && primitives && e.isJsonPrimitive() && matchVal == null) {
				switch (type) {
				case advanced:
					if (!advMatch(term, e.getAsString()))
						break ifc;
					break;
				case normal:
					if (!e.getAsString().toLowerCase().contains(term.toLowerCase()))
						break ifc;
					break;
				case regex:
					if (!Pattern.matches(term, e.getAsString()))
						break ifc;
					break;
				}
		...
		private boolean advMatch(String term, String val) {
			...
			for (int i = 0; i < parts.length; i++) {
				if (!parts[i].equals("")) {
					int index = val.indexOf(parts[i]);
					if (first && index != 0)
						return false;
					if (index == -1)
						return false;
					val = val.substring(index + parts[i].length());
				}
	...
	private static JsonObject search(String arg) {
		int count = 0;
		Search s = new Search();
		boolean pair = false;
		...
		if (arg.contains("-reg ") && arg.contains("-adv ")) {
			System.out.println("cant search advanced and regex simultaneously");
			return null;
		}
		...
		try {
			...
			cb: {
				if (arg.contains("-adv ")) {
					s.setSearchType(SearchType.advanced);
				} else if (arg.contains("-reg ")) {
					s.setSearchType(SearchType.regex);
				} else {
			...
			if (pair) {
				int ind = arg.indexOf(' ');
				s.setTerm(arg.substring(0, ind));
				s.setMatchVal(arg.substring(ind + 1));
			} else {
				s.setTerm(arg);
			}
			ret = s.search(je, path);
		} catch (BothNullException e) {
	...
	private static JsonElement string2JsonElement(String arg) {
		if (arg.startsWith("\\s"))
			return new JsonPrimitive(arg.substring(2));
		switch (arg) {
		case "{}":
	...
	private static void printHelp() {
		...
		String help = "open {file}##opens a file  " + "  " + "cd {obj/arr}##goes into the object/array  "
				+ "cd###goes one back  " + "  " + "help###prints this  " + "  "
				+ "set {key} > {value}#sets the value of a key  "
				+ "###true, false, null and numbers will be added as their type  "
				+ "###{} will add an object, [] will add an array  " + "###ex: set hi > {}  "
				+ "###use \\s before the value to add it as string  " + "###ex: set hi > \\s[]  " + "  "
				+ "rem {key}##deletes this key  " + "  " + "save###saves the json  "
				...
				+ "search [...] {term} [valueTerm, only with -p]#searches after a term  "
				+ "#-n##searches through nested objects/arrays from the current path  "
				+ "#-a##searches through every nested objects/arrays  " + "#-ik##searches only in the keys  "
				+ "#-iv##searches only in the value  " + "#-eo##exludes objects  " + "#-ea##exludes arrays  "
				+ "#-ep##exludes primitives  " + "#-en##exludes nulls  " + "#-p##searches key - value pairs  "
				+ "#-sp##shows the parent object instead of the match, only useful for export  "
		...
		System.out.println(help.replace("\\#", "{hashtag}").replace("#", "\u0009").replace("{hashtag}", "#")
				.replace("  ", "\r\n"));
	}
